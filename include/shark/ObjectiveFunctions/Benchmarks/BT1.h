 ...
 */
//===========================================================================
#ifndef SHARK_OBJECTIVEFUNCTIONS_BENCHMARK_BT1_H
#define SHARK_OBJECTIVEFUNCTIONS_BENCHMARK_BT1_H
...
/**
* \brief Implements the benchmark function BT1.
*
...
* \ingroup benchmarks
*/
struct BT1 : public MultiObjectiveFunction
{
	BT1(std::size_t numVariables = 0) : m_objectives(2), m_handler(numVariables,0,1 ){
	...
	ResultType eval( const SearchPointType & x ) const {
		...
		int n = numberOfVariables();
                ...
                for(int j = 2; j <= n; j++)
                {
                   double yj = x[j-1] - std::sin((j*M_PI)/(2.0*n));
                   if(!(j%2)) sum1 += D1(yj, theta);
                   if((j%2)!=0) sum2 += D1(yj, theta);
               } 
                value[0] = x[0] + sum1;
                value[1] = 1.0 - std::sqrt(x[0]) + sum2;
		return value;
	}
private:
