...
do
  ...
  do
     for seed in {1..35}
     do
     tail -100 $PATH1/POF_${instance}_nobj_2_*_seed_${seed}_px_${px}* | cut -f1,2 -d' '  > POF/${instance}_2_${seed}_${px}
     done
  done
  ...
  do
     for seed in {1..35}
     do
     tail -100 $PATH1/POF_${instance}_nobj_3_*_seed_${seed}_px_${px}* | cut -f1,2,3 -d' '  > POF/${instance}_3_${seed}_${px}
     done
  done
done
